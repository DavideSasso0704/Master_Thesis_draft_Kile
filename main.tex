...
\end{equation}
and can be approximated to $\sim \sqrt{2GMa}$ as the vast majority of stars orbit far from the binary and have a specific angular energy $|E_*|\ll GM/a$.\\Stars of fixed specific energy $E_*$, other than a minimum of the specific angular momentum $J_{min}=J_{lc}(E_*)$, possess a maximum set by the chain of inequalities
\begin{equation}
 E_*\ge \frac{J^2}{2r^2}-\frac{GM}{r}\equiv V_{eff}(r)\ge V_{eff,\,min}=-\frac{G^2M^2}{2J^2},
\end{equation}
which holds because of the non-negative behavior of the radial kinetic term. This constrain poses a limit to $J$, i.e.
\begin{equation}
 J_{max}(E_*)=\frac{GM}{\sqrt{2|E_*|}},
\end{equation}
corresponding to the specific angular momentum of a circular orbit of specific energy $E_*$ \parencite{Capozziello_2008}. Once the binary has removed in a dynamical time $P(E_*)$ the stars inside the influence sphere, the further hardening of the binary via stellar slinghot relies on the rate at which the loss cone is fed by stellar diffusion in both energy and angular momentum space, which causes stars to enter the loss cone.
This diffusion arises from stellar encounters
that have been proven to be efficient in exchanging angular momentum and results in a Random Walk in J-space. Let us define $j_2$ the rms angular momentum transferred in one orbital period  and $J_{max}(E_*)$ the same quantity associated with the timescale $t_{2b}$, only stars with $J_{lc}\le J\le J_{lc}+j_2$ can penetrate into the loss cone over one orbital period.
The mechanism of loss cone replenishing and the associated timescale are set by the dimensionless quantity
\begin{equation}
...
\end{equation}
where $F_{max}(E_*)\sim N(E_*)/P(E_*)$ denotes the maximum rate of loss cone replenishing from stars with specific energy $E_*$ and $\epsilon$ is a correction factor helping us to account for the fraction of stars within the loss cone.\\Let us focus on the full loss cone regime. The 2-D nature of the problem in the J-space suggests that the area correction factor
\begin{equation}
 \epsilon \sim \frac{J_{lc}^2}{(J_{lc}+j_2)^2}
\end{equation}
scales as $\sim J_{lc}^2/J_{max}^2$.\\Therefore, in the pinhole regime, it follows
\begin{equation}\label{stellar flux per unit energy}
 \Gamma_E(E_*)\approx |E_*|^{-1}\frac{a}{r}\frac{N(E_*)}{P(E_*)}.
\end{equation}
From Equation (\ref{stellar flux per unit energy}), we can now derive the stellar flux $F(E_*)$ for an assumed an isothermal sphere of mass $M_*$ and influence radius $r_{inf}$. If all stars have the same mass $m_*$, the total number of stars inside $r$ is simply proportional to tha stellar mass enclosed within this sphere, that is
\begin{equation}
 N(<r)=\frac{M}{m_*}\frac{r}{r_{inf}}.
\end{equation}
Under the assumption of Keplerian orbits bound to the binary, the energy of each star depends on the semimajor only through the relation
\begin{equation}
 E_*(r)=-\frac{GMm_*}{2r},
\end{equation}
while the orbital period is
\begin{equation}
    P(E)=\frac{2\pi GM}{(2|E|)^{3/2}}.
\end{equation}
\parencite{Perets_2007} \parencite{Perets_2008}
\begin{equation}
...
if we set $\Sigma=\pi a^2$

unbound orbit equation
\begin{equation}
 r(\phi)=\frac{l}{1+\cos{\phi}}
\end{equation}
with $l=J^2/GM$ semilatus rectum.

$b$ impact parameter and $v$ velocity at infinity, grav focusing $J=bv$ and $r_P=l/2$ leads to
\begin{equation}
 b^2=\frac{2GMr_P}{v^2}
\end{equation}
...
Full loss cone regime $q\gg 1$



For bound almost radial orbit $dE_*=|E_*|\log{r}$ and
